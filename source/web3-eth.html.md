...
The `web3-eth` package allows you to interact with a Meter blockchain itself and the deployed smart contracts.
...
```javascript
    ...
    // "Web3.givenProvider" will be set if in a Meter supported browser.
...
Property | Type | Description
--------- | ------- | -----------
blockHashOrBlockNumber | `String|Number` | The block number or block hash. Or the string `"genesis"`, `"latest"` or `"pending"` as in the :ref:`default block parameter <eth-defaultblock>`.
...
Property | Type | Description
--------- | ------- | -----------
blockHashOrBlockNumber | `String|Number` | The block number or hash. Or the string `"genesis"`, `"latest"` or `"pending"` as in the :ref:`default block parameter <eth-defaultblock>`.
callback | `Function` | (optional) Optional callback, returns an error object as first parameter and the result as second.
...
.. eth-getpendingtransactions-return:
...
.. eth-gettransactionreceipt-return:
...
address | `String|Number` | Address to sign data with. Or an address or index of a local wallet in :ref:`web3.eth.accounts.wallet <eth_accounts_wallet>`.
callback | `Function` | (optional) Optional callback, returns an error object as first parameter and the result as second.
...
Property | Type | Description
--------- | ------- | ----------
transactionObject | `Object` | The transaction data to sign :ref:`web3.eth.sendTransaction() <eth-sendtransaction>` for more.
...
Property | Type | Description
--------- | ------- | ----------
callObject | `Object` | A transaction object see :ref:`web3.eth.sendTransaction <eth-sendtransaction-return>`, with the difference that for calls the `from` property is optional as well.
defaultBlock | `Number|String` | (optional) If you pass this parameter it will not use the default block set with :ref:`web3.eth.defaultBlock <eth-defaultblock>`.
callback | `Function` | (optional) Optional callback, returns an error object as first parameter and the result as second.
...
storageKey | `Array` 32 Bytes | Array of storage-keys which should be proofed and included. See :ref:`web3.eth.getStorageAt <eth-getStorageAt>`.
blockNumber | `Number | String | "latest" | "earliest"` | Integer block number, or the string "latest" or "earliest".
callback | `Function` | (optional) Optional callback, returns an error object as first parameter and the result as second.
